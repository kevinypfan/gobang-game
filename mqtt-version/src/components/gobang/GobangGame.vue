...
export default {
      ...
      if (topicArray[1] === this.$route.params.id) {
        switch (topicArray[2]) {
          case "pressChess":
            ...
            this.pressMethod(chessPos.x, chessPos.y);
            break;
          case "RoboAction":
  ...
  methods: {
    ...
    reloadGameSetup() {
      this.room.gameChessStep.forEach(step => {
        this.pressMethod(step.x, step.y);
      });
    }
  },
  mounted() {
    this.resetData();
    this.reloadGameSetup();
    var chess = document.getElementById("chess");
    var context = chess.getContext("2d");
    chess.onclick = e => {
      if (this.over) {
      ...
      const message = { x: i, y: j, player: this.player };
